...
class Lapwing:
    ...
    def __init__(self):
        ...
        self.proactive_system = BoredomSystem(
            self.settings,
            BoredomConfig()
        )
        ...
        # State
        self.lapwing_state = load_or_initialize_json(
    ...
    def _load_prompts(self) -> None:
        ...
        try:
            ...
            if world_lore_path.exists():
                self.settings.LAPWING_WORLD_LORE = world_lore_path.read_text(encoding='utf-8')

            persona_path = prompts_dir / "persona.txt"
            if persona_path.exists():
                self.settings.LAPWING_PERSONA_PROMPT = persona_path.read_text(encoding='utf-8')
    ...
    async def start_background_tasks(self):
        ...
        # World updater
        asyncio.create_task(
            self.world_updater.run_loop(interval_minutes=15),
            name="world_updater"
        )
        ...
        asyncio.create_task(
            self.proactive_system.run_loop(
                interval_seconds=60,
                context_provider=self._get_proactive_context
            ),
    ...
    async def _get_proactive_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_active_goals": len(self.proactive_system.goal_manager.get_active_goals()) > 0,
            "active_goal": self.proactive_system.goal_manager.get_random_goal(),
            "memories": list(self.memory_manager.short_term_memory.get("recent_events", []))[-5:],
            "questions": [
                "今天有什么让你开心的事吗？",
                "你在想什么呢？",
                "有什么想和我分享的吗？",
...
Return only a number between -10 and +10.'''
    ...
    async def _extract_memories(self, user_input: str, lapwing_response: str) -> Optional[dict]:
...
Return empty arrays if nothing significant.'''
    ...
    async def _stage_memory(self, user_input: str, lapwing_response: str) -> None:
        ...
        if extracted.get("new_shared_memories"):
            for mem in extracted["new_shared_memories"]:
                self.memory_manager.add_weighted_memory(
                    ...
                    emotional_intensity=abs(self.emotional_state.eii - self.emotional_state.baseline)
                )
                # Emit event
                await emit_memory_added(mem, self.emotional_state.get_eii(), "memory_system")
        ...
        logging.info(f"Staged new memory: {len(staging_data['potential_memories'])} total")
    ...
    async def _handle_session_start(self) -> str:
        ...
        if "eii" in self.lapwing_state:
            self.emotional_state.eii = self.lapwing_state.get("eii", self.settings.EII_BASELINE)
        if "emotional_foundation" in self.lapwing_state:
            self.emotional_state.foundation = self.lapwing_state.get("emotional_foundation", self.settings.EII_BASELINE)
    ...
    def _save_session_state(self) -> None:
        ...
        if abs(old_eii - self.emotional_state.eii) > 5:
            asyncio.create_task(
                emit_eii_changed(old_eii, self.emotional_state.get_eii(), "emotional_state")
            )
    ...
    async def _build_context(self, user_input: str, memory_anchor: str) -> str:
        ...
        long_term_results = await self.memory_manager.retrieve_long_term_memories(
            query=user_input or " ",
            k=3,
            current_eii=self.emotional_state.get_eii()
        )
        long_term_memories = " ".join([content for content, _ in long_term_results]) if long_term_results else ""

        style_exemplars = await self.memory_manager.retrieve_style_exemplars(user_input or " ")

        # Get relevant insights from dreaming system
        insights = self.dreaming_system.get_insights_for_context(
            user_input,
            self.emotional_state.get_eii(),
            n=2
        )
        insight_text = "\n".join([f"- {i.content}" for i in insights]) if insights else ""

        user_time_str = datetime.now(self.user_tz).strftime("%Y-%m-%d %H:%M") if self.user_tz else "Unknown"
    ...
    async def _generate_response(self, context: str, user_input: str) -> str:
        ...
        match = re.search(r'<final_response>(.*?)</final_response>', response, re.DOTALL)
        if match:
            return match.group(1).strip()

        if "<final_response>" in response:
            return response.split("<final_response>")[-1].replace("</final_response>", "").strip()
    ...
    async def get_response(self, user_input: str) -> str:
        try:
            ...
            input_for_model = user_input if user_input.strip() else "[Master is silent, just looking at you quietly.]"
