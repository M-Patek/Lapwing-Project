...
class LapwingConfig(StatelessLLMBaseConfig):
    """Configuration for Lapwing emotional AI backend."""

    base_url: str = Field("http://localhost:8000", alias="base_url")
    model: str = Field("lapwing", alias="model")
    temperature: float = Field(0.95, alias="temperature")
    interrupt_method: Literal["system", "user"] = Field(
        "user", alias="interrupt_method"
    )

    _LAPWING_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "base_url": Description(
            en="Base URL for Lapwing API", zh="Lapwing API 的基础 URL"
        ),
        "model": Description(
...
class StatelessLLMConfigs(I18nMixin, BaseModel):
    ...
    lapwing_llm: LapwingConfig | None = Field(None, alias="lapwing_llm")
    ...
    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        ...
        "lapwing_llm": Description(
            en="Configuration for Lapwing emotional AI", zh="Lapwing 情感 AI 配置"
        ),
    }
