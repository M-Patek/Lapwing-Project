...
class GoalManager:
    ...
    def _load_goals(self):
        """从文件加载目标"""
        data = load_or_initialize_json(self.state_file, {"active": [], "completed": []})
    ...
    def create_goal(
        self, description: str, priority: int = 5, deadline: Optional[datetime] = None
    ) -> Goal:
        """创建新目标"""
        goal = Goal(
...
class BoredomSystem:
    ...
    def __init__(self, settings: Settings, config: Optional[BoredomConfig] = None):
    ...
    def _load_state(self):
        ...
        if data.get("last_proactive_message"):
            self.last_proactive_message = datetime.fromisoformat(
                data["last_proactive_message"]
            )
    ...
    def _save_state(self):
        ...
        data = {
            ...
            "last_proactive_message": self.last_proactive_message.isoformat()
    ...
    def on_user_interaction(self):
        ...
        if old_boredom > 50:
            logging.info(
                f"Boredom reduced by interaction: {old_boredom:.1f} -> {self.boredom:.1f}"
            )
    ...
    def should_trigger_proactive(self) -> bool:
        ...
        if self.last_proactive_message:
            minutes_since_last = (
                datetime.now() - self.last_proactive_message
            ).total_seconds() / 60
    ...
    def generate_message(self, intent: ProactiveIntent, context: Dict) -> str:
        ...
        if intent.intent_type == "share_observation":
            fill_data["observation"] = context.get(
                "current_observation", "刚才发生的事情"
            )
    ...
    def trigger_proactive(self, context: Dict[str, Any]) -> Optional[str]:
        ...
        logging.info(
            f"Proactive triggered: {intent.intent_type} -> '{message[:50]}...'"
        )
    ...
    async def run_loop(
        self, interval_seconds: int = 60, context_provider: Optional[Callable] = None
    ):
        """
        主动行为主循环
