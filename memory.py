...
class MemoryManager:
    def __init__(self, settings: Settings):
        ...
        self.short_term_memory = load_or_initialize_json(self.settings.SHORT_TERM_MEMORY_FILE, {"recent_events": []})
    ...
    def rebuild_long_term_index(self):
        ...
        try:
            ...
            if self.long_term_chunks:
                memory_embeddings = self.embedding_model.embed_contents(content=self.long_term_chunks, task_type="RETRIEVAL_DOCUMENT")["embedding"]
                ...
                logging.info(f"Long-term memory vectorized. Index contains {self.long_term_index.ntotal} chunks.")
            else:
        ...
        try:
            ...
            if style_file.exists() and style_file.stat().st_size > 0:
                ...
                if self.style_exemplars:
                    ...
                    style_embeddings = self.embedding_model.embed_contents(content=style_questions, task_type="RETRIEVAL_DOCUMENT")["embedding"]
                    ...
                    logging.info(f"Style library vectorized. Index contains {self.style_index.ntotal} exemplars.")
        except Exception as e:
    ...
    def get_formatted_working_memory(self) -> str:
        ...
        return "\n".join([f"Master: {turn['user']}\nLapwing: {turn['lapwing']}" for turn in self.working_memory])
    ...
    def get_formatted_short_term_memory(self) -> str:
        ...
        recent_events = self.short_term_memory["recent_events"][-self.settings.STM_RETRIEVAL_K:]
    ...
    def _get_emotional_context_embedding(self, pad_state: dict):
        ...
        p, a, d = pad_state.get('p', 0.5), pad_state.get('a', 0.5), pad_state.get('d', 0.5)
        pleasure_desc = "high pleasure" if p > 0.6 else "low pleasure" if p < 0.4 else "neutral pleasure"
        arousal_desc = "high arousal" if a > 0.6 else "low arousal" if a < 0.4 else "neutral arousal"
        dominance_desc = "high dominance" if d > 0.6 else "low dominance" if d < 0.4 else "neutral dominance"
        context_str = f"Recalling a memory while feeling a sense of {pleasure_desc}, {arousal_desc}, and {dominance_desc}."
        return self.embedding_model.embed_contents(content=[context_str], task_type="RETRIEVAL_QUERY")["embedding"][0]
    ...
    def retrieve_long_term_memories(self, query: str, pad_state: dict) -> str:
        ...
        try:
            query_embedding = np.array(self.embedding_model.embed_contents(content=[query], task_type="RETRIEVAL_QUERY")["embedding"][0])
            emotional_embedding = np.array(self._get_emotional_context_embedding(pad_state))

            # Blend the embeddings
            bias_weight = self.settings.EMOTIONAL_BIAS_WEIGHT
            biased_embedding = (1 - bias_weight) * query_embedding + bias_weight * emotional_embedding
            biased_embedding = biased_embedding.reshape(1, -1) # Reshape for FAISS search

            distances, indices = self.long_term_index.search(biased_embedding, self.settings.LTM_RETRIEVAL_K)
            retrieved_chunks = [self.long_term_chunks[i] for i in indices[0]]

            logging.info(f"Retrieved emotionally-biased long-term memories for query '{query}': {retrieved_chunks}")
    ...
    def retrieve_style_exemplars(self, query: str) -> list[dict]:
        if not self.style_index or not self.style_exemplars:
            return []
        try:
            query_embedding = self.embedding_model.embed_contents(content=[query], task_type="RETRIEVAL_QUERY")["embedding"]
            distances, indices = self.style_index.search(np.array(query_embedding), self.settings.STYLE_RETRIEVAL_K)
            retrieved_exemplars = [self.style_exemplars[i] for i in indices[0]]
            logging.info(f"Retrieved style exemplars for query '{query}': {retrieved_exemplars}")
